...
class Command(BaseCommand):
    ...
    def generate_products(self):
        ...
        for model in all_product_models:
            ...
            for part_name in selected_parts:
                ...
                while self.Product.objects.filter(sku=sku).exists():
                    sku = f"SKU-{random.randint(10000, 99999)}"
    ...
    def generate_repairs(self, users):
        ...
        # Assuming Repair model has fields like: client, device_name, issue_description, status, etc.
        # Since I don't have the exact Repair model definition in front of me, I'll infer common fields.
        # If fields differ, this might need adjustment.
        
        # Based on typical repair models:
        # client (User), device (Product or string), status, description
        ...
        for _ in range(20):
            ...
            repairs.append(
                self.Repair(
                    client=client, # Assuming 'client' field exists and links to User
                    device_name=f"{random.choice(['iPhone', 'Samsung', 'MacBook'])} {random.randint(10, 15)}",
                    issue_description="Screen cracked" if random.choice([True, False]) else "Battery not charging",
                    status=random.choice(statuses),
                    # Add other mandatory fields if any. 
                    # If 'device' is a ForeignKey to Product, I'd need to pick a product.
                    # For now assuming simple text or loose coupling as is common in initial repair tickets.
                )
            )
        
        # Note: bulk_create might fail if there are signals or custom save logic, but usually fine for dummy data
        # If Repair model has auto_now fields, they are handled.
        try:
            self.Repair.objects.bulk_create(repairs)
            self.stdout.write(self.style.SUCCESS(f"{len(repairs)} repairs generated."))
        except Exception as e:
             self.stdout.write(self.style.ERROR(f"Failed to generate repairs: {e}"))
Š²*cascade082efile:///home/amine/coding/web/tek-mag/backend/apps/accounts/management/commands/generate_test_data.py